...
//initialisation for recording 3 seconds if "s" is pressed
unsigned long recordingStartTime = 0;
const unsigned long recordingDuration = 3000; // recording time in Milliseconds (3 seconds)
...
void loop() {
  ...
  if (capture) {
    ...
    if (millis() - recordingStartTime >= recordingDuration) {// check it timer expired
      capture = false; // Stop recording data
  }
}
}
