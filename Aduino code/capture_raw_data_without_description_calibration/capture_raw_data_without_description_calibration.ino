...
void capture_data() {
 ...
 // calculate and print calibrated values
  ...
  int c_thumb = map(Value_thumb, raw_min[0], raw_max[0], streched_value, bent_value);
  int c__indexfinger = map(Value_indexfinger, raw_min[1], raw_max[1], streched_value, bent_value);
  int c_middle = map(Value_middle, raw_min[2], raw_max[2], streched_value, bent_value);
  int c_ring = map(Value_ring, raw_min[3], raw_max[3], streched_value, bent_value);
  int c_pinkie = map(Value_pinkie, raw_min[4], raw_max[4], streched_value, bent_value);
  int c_wrist = map(Value_thumb, raw_min[5], raw_max[5], streched_value, bent_value);
  //print mapped/calibrated values of flex sensors
  Serial.print("c_thumb: ");
  Serial.print(c_thumb);
  Serial.print(",");
  Serial.print("c_indexfinger: ");
  Serial.print(c__indexfinger);
  Serial.print(",");
...
void calibrateSensors() {
  ...
  for (int i = 0; i < num_flexsensor; i++) {
    Serial.print("bent ");
    Serial.println(i+1);
    delay(5000); // Wartezeit, um den Finger zu biegen
    int raw_value = analogRead(finger_pins[i]);
    raw_min[i] = raw_value;
  }
